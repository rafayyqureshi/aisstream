...
def parse_message(record):
    """
    Parsuje AIS (JSON) z Pub/Sub, zwraca dict (lub None, jeśli są braki).
    """
    try:
...
def run():
    ...
    # Subskrypcja AIS (np. "projects/xxx/subscriptions/ais-data-sub-archive")
    input_subscription = pipeline_options.view_as(MyPipelineOptions).input_subscription

    # Domyślna ścieżka do GCS, można nadpisać przez zmienną środowiskową
    output_prefix = os.getenv('HISTORY_OUTPUT_PREFIX', 'gs://ais-collision-detection-bucket/ais_data/raw/ais_raw')
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        # 3) Okienkowanie 5-minutowe + najprostszy trigger (AfterWatermark)
        windowed = (
            parsed
            | 'WindowInto5min' >> beam.WindowInto(
                FixedWindows(5 * 60),        # 5-minutowe okna
